-- ================================================================
-- PARTE 2d — Enriquecer ANOS 2024, 2025, 2026 + restantes
-- ================================================================
...
FROM (
  ...
  WHERE lc.ano_referencia NOT IN (2022, 2023) OR lc.ano_referencia IS NULL
) sub
WHERE tgt.id = sub.id;

SELECT ano_referencia, COUNT(*) AS total, COUNT(drs) AS com_drs
FROM public.lc131_despesas
WHERE ano_referencia NOT IN (2022, 2023) OR ano_referencia IS NULL
